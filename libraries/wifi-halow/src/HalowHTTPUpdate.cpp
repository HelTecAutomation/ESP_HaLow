...
HTTPUpdateResult HTTPUpdate::update(HalowClient& client, const String& url, const String& currentVersion, HTTPUpdateRequestCB requestCB)
{
...
HTTPUpdateResult HTTPUpdate::updateSpiffs(HalowHTTPClient& httpClient, const String& currentVersion, HTTPUpdateRequestCB requestCB)
{
    return handleUpdate(httpClient, currentVersion, true, requestCB);
}

HTTPUpdateResult HTTPUpdate::updateSpiffs(HalowClient& client, const String& url, const String& currentVersion, HTTPUpdateRequestCB requestCB)
{
    HalowHTTPClient http;
    if(!http.begin(client, url))
    {
...
HTTPUpdateResult HTTPUpdate::update(HalowHTTPClient& httpClient,
        const String& currentVersion, HTTPUpdateRequestCB requestCB)
{
    return handleUpdate(httpClient, currentVersion, false, requestCB);
}

HTTPUpdateResult HTTPUpdate::update(HalowClient& client, const String& host, uint16_t port, const String& uri,
        const String& currentVersion, HTTPUpdateRequestCB requestCB)
{
    HalowHTTPClient http;
...
String HTTPUpdate::getLastErrorString(void)
{
    ...
    if(_lastError > -100) {
        return String("HTTP error: ") + HalowHTTPClient::errorToString(_lastError);
    }
...
/**
 *
 * @param http HalowHTTPClient *
 * @param currentVersion const char *
 * @return HTTPUpdateResult
 */
HTTPUpdateResult HTTPUpdate::handleUpdate(HalowHTTPClient& http, const String& currentVersion, bool spiffs, HTTPUpdateRequestCB requestCB)
{
    ...
    case HTTP_CODE_OK:  ///< OK (Start Update)
        if(len > 0) {
            ...
            if(!startUpdate) {
                ...
                HalowClient * tcp = http.getStreamPtr();

// To do?                WiFiUDP::stopAll();
// To do?                HalowClient::stopAllExcept(tcp);
